...
anc_p <- 
read.table('~/ancSH3_paper/SupplementaryMaterial/SupplementaryFiles1/Ancestral_MaxMarginalProb_Char_Indel.txt', header = T)
...
sh3_sequences <- '~/ancSH3_paper/SupplementaryMaterial/Phylo/ortho_seq.afa'
protein_sequences <- system.file("extdata", "sample.fasta", package = "ggmsa")
...
ggmsa(sh3_sequences, font = "DroidSansMono", color = "Chemistry_AA",char_width = 0.5, seq_name = TRUE )+
  geom_msaBar()
ggsave(p1, filename = '~/ancSH3_paper/SupplementaryMaterial/FigurePanels/SuppFig1D.png', width = 12, height = 3, device = 'png')
